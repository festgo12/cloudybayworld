...
<div class="content-area">
                    <div class="mr-breadcrumb">
                        <div class="row">
                            <div class="col-lg-12">
                                <h4 class="heading">Order Invoice<a class="add-btn" href="{{ route('vendor-order-show',$order->order_number) }}"><i class="fas fa-arrow-left"></i> Back</a></h4>
                                <ul class="links">
                                    <li>
                                    ...
                                    <li>
                                        <a href="javascript:;">Order Invoice</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="invoice__logo text-left">
                            <img src="{{asset('assets/images/logo/logo.png')}}" alt="woo commerce logo">
                        </div>
                    </div>
            ...
            <div class="row invoice__metaInfo mb-4">
                <div class="col-lg-6">
                    <div class="invoice__orderDetails">
                        ...
                        <span><strong>Invoice Number :</strong> {{ sprintf("%'.08d", $order->id) }}</span><br>
                        <span><strong>Order Date :</strong> {{ date('d-M-Y',strtotime($order->created_at)) }}</span><br>
                        <span><strong>Order ID :</strong> {{ $order->order_number }}</span><br>
                        @if($order->dp == 0)
                        <span> <strong>Order Details :</strong>
                        ...
                        @endif
                        <span> <strong>Payment Method :</strong> {{$order->method}}</span>
                    </div>
                </div>
            </div>
           ...
           @if($order->dp == 0)
                <div class="col-lg-6">
                        <div class="invoice__shipping">
                           ...
                           <span><strong>Customer Name</strong>: {{ $order->shipping_name == null ? $order->customer_name : $order->shipping_name}}</span><br>
                           <span><strong>Address</strong>: {{ $order->shipping_address == null ? $order->customer_address : $order->shipping_address }}</span><br>
                           <span><strong>City</strong>: {{ $order->shipping_city == null ? $order->customer_city : $order->shipping_city }}</span><br>
                           <span><strong>Country</strong>: {{ $order->shipping_country == null ? $order->customer_country : $order->shipping_country }}</span>
            ...
            @endif
                ...
                <div class="col-lg-6">
                        <div class="buyer">
                            ...
                            <span><strong>Address</strong>: {{ $order->customer_address }}</span><br>
                            <span><strong>City</strong>: {{ $order->customer_city }}</span><br>
                            <span><strong>Country</strong>: {{ $order->customer_country }}</span>
                        </div>
                </div>
            </div>
                            ...
                            <div class="table-responsive">
                                    ...
                                    @if($product['item']['user_id'] == $user->id)
                                        ...
                                        <tr>
                                            <td width="50%">
                                                ...
                                                @if(isset($user))
                                                <a target="_blank"
                                                    href="{{ route('product.details', $product['item']['slug']) }}">{{ $product['item']['name']}}</a>
                                                @else
                                ...
                                @endif
                                    ...
                                    <tfoot>
                                        ...
                                        @if($order->tax != 0)
                                        <tr>
                                            <td colspan="2">TAX ({{$order->currency_sign}})</td>
                                            @php
