  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".messenger-infoView .following-num").html(data.following);
        $(".messenger-infoView .followers-num").html(data.followers);
        if(data.fetch.userBio)
        {

          $(".messenger-infoView .user-about").html(data.fetch.userBio);
        }else{
          $(".messenger-infoView .user-about").html('About not Set');

        }

        $(".m-header-messaging .user-name").html(data.fetch.username);

        if(data.fetch.active_status)
        {
          $(".messenger-infoView .status").html('Active').addClass('text-success').removeClass('text-danger');
          
        }else{
          
          $(".messenger-infoView .status").html('Not Active').addClass('text-danger').removeClass('text-success');
        }
        // Star status
        data.favorite > 0
